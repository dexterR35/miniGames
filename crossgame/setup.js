...
//          
//==============================================================================
// if (selectedTeams.includes(selectedword)) {
//     alert("You have selected a word that is in the array!");
// }
(function ($, undefined) {
    ...
    $.widget("ryanf.wordsearchwidget", $.ui.mouse, {
        ...
        _mapEventToCell: function (event) {
            let currentColumn = Math.ceil((event.pageX - this._cellX) / this._cellWidth);
            let currentRow = Math.ceil((event.pageY - this._cellY) / this._cellHeight);
            let el = $('#rf-tablegrid tr:nth-child(' + currentRow + ') td:nth-child(' + currentColumn + ')');
            return el;
        },
    ...
    function Arms() {
        ...
        this.returnToNormal = function () {
            ...
            for (let t = 1; t < this.arms.length; t++) { //don't clear the hotzone
    ...
    function Grid() {
        ...
        this.put = function (row, col, word) {
            ...
            let populator = eval("new " + eval("this.directions[" + Math.floor(Math.random() * 4) + "]") + "Populator(row,col,word, this)");
            let isPlaced = populator.populate();

            //Didn't get placed.. brute force-fit (if possible)
            if (!isPlaced) {
                for (let x = 0; x < this.directions.length; x++) {
                    let populator2 = eval("new " + eval("this.directions[" + x + "]") + "Populator(row,col,word, this)");
    ...
    function VerticalPopulator(row, col, word, grid) {
        ...
        this.willWordFit = function () {
            ...
            let freeCounter = 0;
            let chars = this.word.chars;
            for (let i = row; i < this.size; i++) { // CHANGED
        ...
        this.populate = function () {
            ...
            if (this.willWordFit()) {
                ...
                if (output[0] != true) {
                    ...
                    // for every row - try to fit this
                    OUTER: for (let col = 0, row = (this.size - word.size); row >= 0; row--) {
                        for (let j = 0; j < 2; j++) {

                            let op = this.findContigousSpace((j == 0) ? row : col, (j == 0) ? col : row, word);
        ...
        this.populate = function () {
            ...
            if (this.willWordFit()) {
                ...
                if (output[0] != true) {
                    ...
                    // for every row - try to fit this
                    OUTER: for (let col = this.size - 1, row = (this.size - word.size); row >= 0; row--) {
                        for (let j = 0; j < 2; j++) {

                            let op = this.findContigousSpace((j == 0) ? row : (this.size - 1 - col), (j == 0) ? col : (this.size - 1 - row), word);
    ...
    let GameWidgetHelper = {
        ...
        renderGame: function (container, model) {
            ...
            let puzzleGrid = "<div id='rf-searchgamecontainer'><table id='rf-tablegrid' cellspacing=0 cellpadding=0 class='rf-tablestyle'>";
